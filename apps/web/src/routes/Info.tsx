...
const SEARCH_INDEX: readonly SearchEntry[] = [
  ...
  { tab: "contracts", title: "If you scan these yourself",       body: "What automated scanner flags mean — documented false positives + controlled patterns walked through: public burn x3, incorrect access control, _safeMint reentrancy, setGuardians missing modifier, OZ-library unchecked-block, Alerts controlled low-level call.", keywords: "solidityscan false positive scanner" },
  ...
  return (
    <section className="space-y-6">
      ...
      <div id="bug-bounty" className="card border-accent-green/50">
        ...
        <ul className="list-disc list-inside space-y-1 text-sm text-text-on-dark/90">
          <li>The known false-positive findings already documented in "If you scan these yourself" below — public-burn × 3 on the SBT, setGuardians-missing-modifier on Recovery, unchecked-block in flattened OZ library code, and Alerts' controlled low-level call on its owner-scoped self-funded POL ping.</li>
      ...
      <div className="card">
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="border-l-2 border-amber-400/50 pl-3">
            <div className="font-bold text-amber-300">"Controlled low-level call" (Alerts, 1× Critical) — controlled, non-exploitable</div>
            <p className="text-text-muted mt-1">NoKLockAlerts (the "Live-Man's Switch") sends a tiny 0.001 POL dust ping to <strong>your own pre-registered</strong> watcher wallets when a recovery starts against you — the guaranteed record is the on-chain <code>AlertPinged</code> event; the dust is just a wallet-app nudge. SolidityScan flags the native-POL <code>.call</code> as Critical generically. It is controlled by construction: the destination is <strong>always the owner's own registered watcher</strong>, never caller-controlled (a malicious keeper can at worst burn your own dust to your own wallets — which only helps you); the amount is fixed dust drawn from <strong>your own self-funded escrow</strong> (the contract holds no pooled balance); and the send is <strong>effects-before-interaction, <code>nonReentrant</code>, gas-bounded (30k) and success-ignored</strong> — a reverting or greedy watcher contract can't brick the keeper, drain gas, or re-enter. Removing the low-level send would remove the wallet-app alert itself.</p>
          </div>
          <div className="border-l-2 border-amber-400/50 pl-3">
